...
class LLM_Solver():
    ...
    def _init_filter_llm(self):
        """
        Initializes the LLM model that connects to the pace cluster with NIM. This instance will handle only filtering.
        ...
        self.filter_llm = ChatNVIDIA(base_url=server, model="meta/llama-3.3-70b-instruct")
        print('Filter LLM Connected to Cluster')

    def _init_llm(self):
        """
        Initializes the LLM model that connects to the pace cluster with NIM.

        This method is used to set up the LLM model that will be used for generating code or reasoning based on input.
        """
        # Initialize LLM
        server = 'http://localhost:8000/v1'#/chat/completions'
        self.llm = ChatNVIDIA(base_url=server, model="meta/llama-3.3-70b-instruct")
        print('Main LLM Connected to Cluster')
